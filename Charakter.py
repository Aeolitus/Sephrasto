...
class KampfstilMod:
    def __init__(self):
        ...
        self.BEIgnore = (
            []
        )  # Tupel aus Kampffertigkeit und Talent für welche die BE ignoriert wird
...
class Char:
    ...
    def __init__(self):
        ...
        self.freieFertigkeitenNumKostenlos = EventBus.applyFilter(
            "freiefertigkeit_num_kostenlos", 1
        )
        ...
        self.charakterScriptAPI = {
            ...
            "getÜbernatürlicheFertigkeit": lambda name: copy.deepcopy(
                self.übernatürlicheFertigkeiten[name]
            ),
            "getFreieFertigkeiten": lambda: copy.deepcopy(self.freieFertigkeiten),
            "getVorteile": lambda: copy.deepcopy(
                [el for el in Wolke.DB.vorteile if el in self.vorteile]
            ),
            ...
            "modifyFertigkeitBasiswert": lambda name, mod: setattr(
                self.fertigkeiten[name],
                "basiswertMod",
                self.fertigkeiten[name].basiswertMod + mod,
            ),
            ...
            "modifyAsPBasis": lambda aspBasis: setattr(
                self, "aspBasis", self.aspBasis + aspBasis
            ),
            "getAsPMod": lambda: self.aspMod,
            "setAsPMod": lambda aspMod: setattr(self, "aspMod", aspMod),
            "modifyAsPMod": lambda aspMod: setattr(
                self, "aspMod", self.aspMod + aspMod
            ),
            # Kap
            "getKaPBasis": lambda: self.kapBasis,
            "setKaPBasis": lambda kapBasis: setattr(self, "kapBasis", kapBasis),
            "modifyKaPBasis": lambda kapBasis: setattr(
                self, "kapBasis", self.kapBasis + kapBasis
            ),
            "getKaPMod": lambda: self.kapMod,
            "setKaPMod": lambda kapMod: setattr(self, "kapMod", kapMod),
            "modifyKaPMod": lambda kapMod: setattr(
                self, "kapMod", self.kapMod + kapMod
            ),
            # Schip
            "getSchiPMax": lambda: self.schipsMax,
            "setSchiPMax": lambda schipsMax: setattr(self, "schipsMax", schipsMax),
            "modifySchiPMax": lambda schipsMax: setattr(
                self, "schipsMax", self.schipsMax + schipsMax
            ),
            # WS
            ...
            "setSchadensbonus": lambda schadensbonus: setattr(
                self, "schadensbonus", schadensbonus
            ),
            "modifySchadensbonus": lambda schadensbonus: setattr(
                self, "schadensbonus", self.schadensbonus + schadensbonus
            ),
            # INI
            ...
            "modifyBEMod": lambda beMod: setattr(
                self, "rüstungsgewöhnung", self.rüstungsgewöhnung + beMod
            ),
            # Kampfstil
            ...
            "setKampfstilBEIgnore": lambda kampfstil, fertigkeit, talent: self.kampfstilMods[
                kampfstil
            ].BEIgnore.append(
                [fertigkeit, talent]
            ),
        ...
        for attribut in self.attribute.values():
            self.charakterScriptAPI[
                "get" + attribut.key
            ] = lambda attribut=attribut.key: self.attribute[attribut].wert

        self.waffenScriptAPI = {
            "getEigenschaftParam": lambda paramNb: self.API_getWaffeneigenschaftParam(
                paramNb
            ),
            "modifyWaffeAT": lambda atmod: setattr(
                self.currentWaffenwerte, "AT", self.currentWaffenwerte.AT + atmod
            ),
            "modifyWaffeVT": lambda vtmod: setattr(
                self.currentWaffenwerte, "VT", self.currentWaffenwerte.VT + vtmod
            ),
            "modifyWaffeTPW6": lambda tpw6mod: setattr(
                self.currentWaffenwerte, "TPW6", self.currentWaffenwerte.TPW6 + tpw6mod
            ),
            "modifyWaffeTPPlus": lambda tpplusmod: setattr(
                self.currentWaffenwerte,
                "TPPlus",
                self.currentWaffenwerte.TPPlus + tpplusmod,
            ),
            "modifyWaffeHaerte": lambda haertemod: setattr(
                self.currentWaffenwerte,
                "Haerte",
                self.currentWaffenwerte.Haerte + haertemod,
            ),
            "modifyWaffeRW": lambda rwmod: setattr(
                self.currentWaffenwerte, "RW", self.currentWaffenwerte.RW + rwmod
            ),
            ...
            "setWaffeTPPlus": lambda tpplus: setattr(
                self.currentWaffenwerte, "TPPlus", tpplus
            ),
            "setWaffeHaerte": lambda haerte: setattr(
                self.currentWaffenwerte, "Haerte", haerte
            ),
    ...
    def API_modifyKampfstil(self, kampfstil, at, vt, tp, rw, be=0):
        k = self.kampfstilMods[kampfstil]
        self.API_setKampfstil(
            kampfstil, k.AT + at, k.VT + vt, k.TP + tp, k.RW + rw, k.BE + be
        )
    ...
    def API_getWaffeneigenschaftParam(self, paramNb):
        ...
        if not len(parameters) >= paramNb:
            raise Exception(
                ...
                + "' erfordert "
                + paramNb
                + " Parameter, aber es wurden nur "
                + len(parameters)
                + " gefunden. Parameter müssen mit Semikolon getrennt werden"
            )
    ...
    def updateWaffenwerte(self):
        ...
        for el in self.waffen:
            ...
            if el.kampfstil in self.kampfstilMods:
                ...
                if el.kampfstil != Definitionen.KeinKampfstil:
                    logging.warn(
                        "Waffe "
                        + el.name
                        + " referenziert einen nicht existierenden Kampfstil: "
                        + el.kampfstil
                    )
            ...
            schadensbonusWirkt = (
                type(el) == Objekte.Nahkampfwaffe or fertig == "Wurfwaffen"
            )
            schadensbonusWirkt = EventBus.applyFilter(
                "waffe_schadensbonus_wirkt", schadensbonusWirkt, {"waffe": el}
            )
            ...
            for key in sorted(eigenschaftenByPrio):
                for weName in eigenschaftenByPrio[key]:
                    ...
                    logging.info(
                        "Character: applying script for Waffeneigenschaft " + weName
                    )
                    we = Hilfsmethoden.GetWaffeneigenschaft(weName, Wolke.DB)
    ...
    def epZaehlen(self):
        ...
        for key in Definitionen.Attribute:
            val = (
                sum(range(self.attribute[key].wert + 1))
                * self.attribute[key].steigerungsfaktor
            )
            spent += EventBus.applyFilter(
                "attribut_kosten",
                val,
                {"attribut": key, "wert": self.attribute[key].wert},
            )
        ...
        for fer in self.fertigkeiten:
            val = (
                sum(range(self.fertigkeiten[fer].wert + 1))
        ...
        for fer in self.freieFertigkeiten:
            ...
            if not fer.name:
                continue
            val = EventBus.applyFilter(
                "freiefertigkeit_kosten",
                Definitionen.FreieFertigkeitKosten[fer.wert - 1],
                {"name": fer.name, "wert": fer.wert},
            )
            spent += val
        ...
        for fer in self.übernatürlicheFertigkeiten:
            val = (
                sum(range(self.übernatürlicheFertigkeiten[fer].wert + 1))
            ...
            for tal in self.übernatürlicheFertigkeiten[fer].gekaufteTalente:
                if tal in paidTalents:
                    continue
                paidTalents.append(tal)
                val = self.getTalentCost(
                    tal, self.übernatürlicheFertigkeiten[fer].steigerungsfaktor
                )
                spent += val
        ...
        while True:
            ...
            for vor in self.vorteile:
                ...
                if not self.voraussetzungenPrüfen(
                    Wolke.DB.vorteile[vor].voraussetzungen
                ):
                    remove.append(vor)
                    contFlag = False
            for el in remove:
    ...
    def findUnerfüllteVorteilVoraussetzungen(
        self,
        vorteile=None,
        waffen=None,
        attribute=None,
        übernatürlicheFertigkeiten=None,
        fertigkeiten=None,
    ):
        """
        Checks for all Vorteile if the requirements are still met until in one
        ...
        übernatürlicheFertigkeiten = (
            übernatürlicheFertigkeiten or self.übernatürlicheFertigkeiten
        )
    ...
    def updateFerts(self):
        ...
        while True:
            ...
            for fert in self.fertigkeiten:
                if not self.voraussetzungenPrüfen(
                    self.fertigkeiten[fert].voraussetzungen
                ):
                    remove.append(fert)
                    contFlag = False
        ...
        for fert in self.fertigkeiten:
            ...
            if self.fertigkeiten[fert].wert > self.fertigkeiten[fert].maxWert:
                ...
                self.fertigkeiten[fert].aktualisieren()
            if (
                self.fertigkeiten[fert].kampffertigkeit == 1
                and self.fertigkeiten[fert].wert > self.höchsteKampfF
            ):
        ...
        for fert in self.übernatürlicheFertigkeiten:
            ...
            if (
                ...
                self.übernatürlicheFertigkeiten[
                    fert
                ].wert = self.übernatürlicheFertigkeiten[fert].maxWert
                self.übernatürlicheFertigkeiten[fert].aktualisieren()
    ...
    def voraussetzungenPrüfenInternal(
        vorteile,
        waffen,
        attribute,
        übernatürlicheFertigkeiten,
    ...
    def xmlSchreiben(self, filename):
        ...
        etree.SubElement(versionXml, "DatenbankVersion").text = str(
            self.datenbankCodeVersion
        )
        etree.SubElement(versionXml, "NutzerDatenbankCRC").text = (
            str(binascii.crc32(etree.tostring(Wolke.DB.userDbXml)))
        ...
        for vort in self.vorteile:
            ...
            if vort in self.vorteileVariable:
                v.set(
                    "variable",
                    str(self.vorteileVariable[vort].kosten)
                    + ","
                    + self.vorteileVariable[vort].kommentar,
                )
            else:
                v.set("variable", "-1")
        # Vierter Block
        ...
        for fert in self.fertigkeiten:
            ...
            for talent in self.fertigkeiten[fert].gekaufteTalente:
                ...
                if talent in self.talenteVariable:
                    talNode.set(
                        "variable",
                        str(self.talenteVariable[talent].kosten)
                        + ","
                        + self.talenteVariable[talent].kommentar,
                    )
                else:
        ...
        for fert in self.übernatürlicheFertigkeiten:
            ...
            fertNode.set(
                "addToPDF", str(self.übernatürlicheFertigkeiten[fert].addToPDF)
            )
            talentNode = etree.SubElement(fertNode, "Talente")
    ...
    def charakterMigrieren(self, xmlRoot, charDBVersion, datenbankCodeVersion):
        ...
        while charDBVersion < datenbankCodeVersion:
            logging.warning(
                "Migriere Charakter von Version "
                + str(charDBVersion)
                + " zu "
                + str(charDBVersion + 1)
            )
        ...
        if dbChanged:
            ...
            messageBox.setText(
                "Seit du diesen Charakter das letzte mal bearbeitet hast wurde die offizielle Sephrasto-Datenbank aktualisiert. "
    ...
    def xmlLesen(self, filename):
        ...
        if versionXml is not None:
            ...
            userDBName = (
                versionXml.find("NutzerDatenbankName").text or "Keine Nutzer-Regelbasis"
            )
            if Wolke.DB.userDbXml is not None:
            ...
            for tal in fer.findall("Talente/Talent"):
                ...
                if var:
                    ...
                    if Wolke.DB.talente[nam].variableKosten:
                        defaultKosten = self.getDefaultTalentCost(
                            nam, fert.steigerungsfaktor
                        )
                        var.kosten = max(
                            var.kosten - (var.kosten % defaultKosten), defaultKosten
                        )
                    else:
                        var.kosten = self.getDefaultTalentCost(
                            nam, fert.steigerungsfaktor
                        )
        ...
        for waf in objekte.findall("Waffen/Waffe"):
            ...
            if waf.attrib["eigenschaften"]:
                waff.eigenschaften = list(
                    map(str.strip, waf.attrib["eigenschaften"].split(","))
                )
        ...
        if userDBChanged or vIgnored or fIgnored or tIgnored or übIgnored:
            ...
            messageBox.setWindowTitle(
                "Charakter laden - Nutzer-Datenbank wurde geändert."
            )

            strArr = [
                "Seit du diesen Charakter das letzte mal bearbeitet hast wurde die Nutzer-Datenbank aktualisiert. "
            ]
            if not userDBName == Wolke.DB.datei:
                strArr.append(
                    "Auch der Pfad der aktuell geladenen Nutzer-Datenbank ist ein anderer:\n- Vorher: '"
                )
            ...
            strArr.append(
                "Neu zur Datenbank hinzugefügte Dinge sind unproblematisch. Waffen werden nicht automatisch angepasst und behalten ihren (eventuell alten) Stand. "
            )
            strArr.append(
                "Dein Charakter ist jetzt an die aktuelle Nutzer-Datenbank angepasst, überspeichere den Charakter nur wenn du dir sicher bist."
            )
            text = "".join(strArr)
