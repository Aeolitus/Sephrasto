...
class Hilfsmethoden:
    ...
    def GetWaffeneigenschaft(WaffeneigenschaftStr, Datenbank):
        ...
        if not weName in Datenbank.waffeneigenschaften:
            raise WaffeneigenschaftException(
                "Unbekannte Waffeneigenschaft '" + weName + "'"
            )
        ...
        if index != -1:
            ...
            if endIndex == -1:
                raise WaffeneigenschaftException(
                    "Parameter der Waffeneigenschaft '"
                    + weName
                    + "' müssen mit ')' abgeschlossen werden. Mehrere Parameter werden mit Semikolon getrennt."
                )
    ...
    def FertStr2Array(FertString, Datenbank=None):
        ...
        for itm in FertString.split(","):
            ...
            if len(strpItm) > 0:
                if (
                    (Datenbank is None)
                    or (strpItm in Datenbank.fertigkeiten)
                    or (strpItm in Datenbank.übernatürlicheFertigkeiten)
                ):
            ...
            if " ODER " in strpItm:
                ...
                if strpItm.startswith("Vorteil "):
                    if not (strpItm[8:] in Datenbank.vorteile):
                        raise VoraussetzungException(
                            "Kann Vorteil '"
                            + strpItm
                            + "' in der Datenbank nicht finden."
                        )
                    ...
                    if attribut in Definitionen.Attribute:
                        ...
                        raise VoraussetzungException(
                            ...
                            + attribut
                            + "' ist ungültig. Unterstützt werden 'KO', 'MU', 'GE', 'KK', 'IN', 'KL', 'CH' und 'FF'"
                        )
                elif strpItm.startswith("Übernatürliche-Fertigkeit "):
                    if not strpItm[26] == "'":
                        raise VoraussetzungException(
                            "Der Name einer Übernatürlichen Fertigkeit muss in Apostrophen gefasst werden. ("
                            + strpItm
                            + ")"
                        )
                    strpItm = strpItm[27:]
                    ...
                    if not (fertigkeit in Datenbank.übernatürlicheFertigkeiten):
                        raise VoraussetzungException(
                            "Kann Übernatürliche Fertigkeit '"
                            + fertigkeit
                            + "' in der Datenbank nicht finden."
                        )
                    try:
                        ...
                        raise VoraussetzungException(
                            "Der angegebene Fertigkeitswert '"
                            + strpItm[index + 2 :]
                            + "' ist keine gültige Zahl"
                        )
                    ...
                    if not strpItm[11] == "'":
                        raise VoraussetzungException(
                            "Der Name einer Fertigkeit muss in Apostrophen gefasst werden. . ("
                            + strpItm
                            + ")"
                        )
                    strpItm = strpItm[12:]
                    ...
                    raise VoraussetzungException(
                        ...
                        + strpItm
                        + "'. Unterstützt werden 'Vorteil', 'Kein Vorteil', 'Waffeneigenschaft', 'Attribut', 'Übernatürliche-Fertigkeit' und 'Fertigkeit'."
                    )
