...
CharakterbogenInfo = namedtuple(
    "CharakterbogenInfo",
    "filePath maxVorteile maxFreie maxFertigkeiten seitenProfan kurzbogenHack",
)
...
class pdfMeister(object):
    ...
    def pdfErstellen(self, filename, printRules):
        ...
        if self.UseExtraPage:
            while (
                len(self.ExtraVorts) > 0
                or len(self.ExtraTalents) > 0
                or len(self.ExtraUeber) > 0
            ):
                fieldsNew = None
                extraPageAdded = True
                Wolke.Fehlercode = -91
                ...
                Wolke.Fehlercode = -92
                fieldsNew = self.createExtra(
                    self.ExtraVorts, self.ExtraUeber, self.ExtraTalents, fieldsNew
                )
                Wolke.Fehlercode = -93
        ...
        if (
            not ("Uebervorteil1" in fields)
            and not ("Ueberfer1NA" in fields)
            and not ("Uebertal1NA" in fields)
            and not extraPageAdded
        ):
            Wolke.Fehlercode = -96
            handle, out_file = tempfile.mkstemp()
            os.close(handle)
        ...
        if printRules:
            ...
            while startIndex != -1:
                ...
                str, startIndex = self.CheatsheetGenerator.writeRules(
                    rules, ruleLineCounts, startIndex
                )
    ...
    def pdfZweiterBlock(self, fields):
        ...
        if isZauberer:
            ...
            fields["Astralenergie"] = (
                Wolke.Char.asp.wert + Wolke.Char.aspBasis + Wolke.Char.aspMod
            )
        ...
        if isGeweiht:
            ...
            fields["Karmaenergie"] = (
                Wolke.Char.kap.wert + Wolke.Char.kapBasis + Wolke.Char.kapMod
            )
    ...
    def pdfDritterBlock(self, fields):
        ...
        for vort in sortV:
            ...
            name = self.CheatsheetGenerator.getLinkedName(
                Wolke.DB.vorteile[vort], forceKommentar=True
            )
            removed.append(vort)
            added.append(name)
        ...
        if len(tmpVorts) > self.CharakterBogen.maxVorteile * mergePerLineCount:
            tmpOverflow.extend(
                tmpVorts[self.CharakterBogen.maxVorteile * mergePerLineCount :]
            )
        ...
        counter = 0
        for i in range(
            len(tmpVorts), self.CharakterBogen.maxVorteile * mergePerLineCount
        ):
        ...
        for i in range(
            0, min(self.CharakterBogen.maxVorteile * mergePerLineCount, len(tmpVorts))
        ):
    ...
    def writeFertigkeiten(self, fields, baseStr, fertigkeitenNames):
        ...
        for el in fertigkeitenNames:
            ...
            for el2 in talente:
                ...
                talStr += talent.getFullName(Wolke.Char).replace(
                    fertigkeit.name + ": ", ""
                )
    ...
    def pdfFünfterBlock(self, fields):
        ...
        for el in Wolke.Char.rüstung:
            ...
            fields[base + "WS"] = int(
                el.getRSGesamtInt() + Wolke.Char.rsmod + Wolke.Char.ws
            )
            base += "RS"
        ...
        for el in Wolke.Char.waffen:
            ...
            fields[base + "TP"] = (
                "-" if keinSchaden else str(el.W6) + "W6" + sg + str(el.plus)
            )
            ...
            vtErlaubt = not (
                type(el) == Objekte.Fernkampfwaffe
                or (
                    el.name in Wolke.DB.waffen
                    and Wolke.DB.waffen[el.name].talent == "Lanzenreiten"
                )
            )
            vtErlaubt = EventBus.applyFilter(
                "waffe_vt_erlaubt", vtErlaubt, {"waffe": el}
            )
            if vtErlaubt:
            ...
            fields[base + "TPm"] = (
                "-"
                if keinSchaden
                else str(waffenwerte.TPW6) + "W6" + sg + str(waffenwerte.TPPlus)
            )
    ...
    def pdfSechsterBlock(self, fields):
        ...
        for f in Wolke.Char.übernatürlicheFertigkeiten:
            if (
                Wolke.Char.übernatürlicheFertigkeiten[f].wert > 0
                or len(Wolke.Char.übernatürlicheFertigkeiten[f].gekaufteTalente) > 0
            ):
                talsList.extend(
                    Wolke.Char.übernatürlicheFertigkeiten[f].gekaufteTalente
                )
                countFerts += 1
        ...
        for f in fertsList:
            ...
            if fe.addToPDF:
                if countF < 13:
                    ...
                    fields[base + "AT"] = (
                        fe.attribute[0] + "/" + fe.attribute[1] + "/" + fe.attribute[2]
                    )
