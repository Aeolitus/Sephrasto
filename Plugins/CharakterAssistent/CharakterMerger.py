...
from CharakterAssistent import Choice, ChoicePopupWrapper, VariantPopupWrapper
...
class CharakterMerger(object):
    ...
    def addFreieFertigkeit(name, wert, overrideEmpty):
        ...
        if not found:
            ...
            if (
                overrideEmpty
                and fert.wert == 3
                and len(char.freieFertigkeiten) > 0
                and char.freieFertigkeiten[0].name == ""
            ):
    ...
    def readChoices(path):
        ...
        for varianten in root.findall("Varianten"):
            ...
            if "pflichtwahl" in varianten.attrib:
                variantListCollection.chooseOne = (
                    int(varianten.attrib["pflichtwahl"]) != 0
                )
            else:
                variantListCollection.chooseOne = False
            CharakterMerger.xmlNodeToChoices(
                variantListCollection, varianten.findall("Variante")
            )
    ...
    def handleChoices(paths, title, geschlecht, spezies, kultur, profession):
        ...
        if not spezies:
            ...
            if kultur:
                ...
                EventBus.doAction(
                    "cbext_update", {"name": "profession", "value": title}
                )
        ...
        variantListCollections, choiceListCollection = CharakterMerger.readChoices(
            paths[1]
        )
        ...
        for variantListCollection in variantListCollections:
            variantListCollection.choiceLists = [
                cl
                for cl in variantListCollection.choiceLists
                if cl.meetsConditions(geschlecht, variantsSelected)
            ]
            ...
            if (
                variantListCollection.chooseOne
                and len(variantListCollection.choiceLists) == 1
            ):
                choices.append(0)
            else:
                popup = VariantPopupWrapper.VariantPopupWrapper(
                    variantListCollection, title
                )
        ...
        if len(description) > 0:
            ...
            if spezies:
                ...
                EventBus.doAction(
                    "cbext_update",
                    {"name": "kultur", "value": title + " (" + description + ")"},
                )
            elif profession:
                char.kurzbeschreibung += " (" + description + ")"
                EventBus.doAction(
                    "cbext_update",
                    {"name": "profession", "value": title + " (" + description + ")"},
                )
    ...
    def xmlNodeToChoices(choiceListCollection, node):
        for child in node:
            ...
            if "varianten" in child.attrib:
                choiceList.varianten = [
                    int(x) for x in child.attrib["varianten"].split(",")
                ]
            if "keine-varianten" in child.attrib:
                choiceList.keineVarianten = [
                    int(x) for x in child.attrib["keine-varianten"].split(",")
                ]
            ...
            if "geschlecht" in child.attrib:
                ...
                if geschlecht != "männlich" and geschlecht != "weiblich":
                    logging.warn(
                        "CharakterAssistent: Unbekanntes Geschlecht " + geschlecht
                    )
                else:
            ...
            for fer in child.findall("Attribut"):
                ...
                if not choice.name in Definitionen.Attribute.keys():
                    logging.warn(
                        "CharakterAssistent: konnte Attribut "
                        + choice.name
                        + " nicht finden"
                    )
                    continue
            ...
            for fer in child.findall("Fertigkeit"):
                ...
                if not choice.name in Wolke.DB.fertigkeiten:
                    logging.warn(
                        "CharakterAssistent: konnte Fertigkeit "
                        + choice.name
                        + " nicht finden"
                    )
                    continue
            ...
            for fer in child.findall("Übernatürliche-Fertigkeit"):
                ...
                if not choice.name in Wolke.DB.übernatürlicheFertigkeiten:
                    logging.warn(
                        "CharakterAssistent: konnte Übernatürliche Fertigkeit "
                        + choice.name
                        + " nicht finden"
                    )
                    continue
            ...
            for vor in child.findall("Vorteil"):
                ...
                if not choice.name in Wolke.DB.vorteile:
                    logging.warn(
                        "CharakterAssistent: konnte Vorteil "
                        + choice.name
                        + " nicht finden"
                    )
                    continue
            ...
            for vor in child.findall("Talent"):
                ...
                if not choice.name in Wolke.DB.talente:
                    logging.warn(
                        "CharakterAssistent: konnte Talent "
                        + choice.name
                        + " nicht finden"
                    )
                    continue
    ...
    def applyChoiceToChar(choice):
        ...
        if choice.typ == "Eigenheit":
            ...
            char.fertigkeiten[choice.name].wert = max(
                char.fertigkeiten[choice.name].wert + choice.wert, 0
            )
            char.fertigkeiten[choice.name].aktualisieren()
        elif choice.typ == "Übernatürliche-Fertigkeit":
            char.übernatürlicheFertigkeiten[choice.name].wert = max(
                char.übernatürlicheFertigkeiten[choice.name].wert + choice.wert, 0
            )
            ...
            for fert in Wolke.DB.talente[choice.name].fertigkeiten:
                ...
                if fert in char.übernatürlicheFertigkeiten:
                    char.übernatürlicheFertigkeiten[fert].gekaufteTalente.append(
                        choice.name
                    )
    ...
    def xmlLesen(path, spezies, kultur):
        ...
        for atr in root.findall("Attribute/*"):
            if not spezies and not kultur:
                char.attribute[atr.tag].wert = max(
        ...
        for fer in root.findall("Fertigkeiten/Freie-Fertigkeit"):
            CharakterMerger.addFreieFertigkeit(
                fer.attrib["name"], int(fer.attrib["wert"]), False
            )
