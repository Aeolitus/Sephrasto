...
class EinstellungenWrapper:
        ...
        if self.ret == QtWidgets.QDialog.Accepted:
            ...
            for checkbox in self.pluginCheckboxes:
                if checkbox.isChecked() and (
                    checkbox.text() in Wolke.Settings["Deaktivierte-Plugins"]
                ):
                    Wolke.Settings["Deaktivierte-Plugins"].remove(checkbox.text())
                elif not checkbox.isChecked() and not (
                    checkbox.text() in Wolke.Settings["Deaktivierte-Plugins"]
                ):
        ...
        if os.path.isfile(settingsPath):
            with open(settingsPath, "r") as infile:
                ...
                if Wolke.Settings["Version"] == 0:
                    if (
                        not "CharakterBeschreibungExt"
                        in Wolke.Settings["Deaktivierte-Plugins"]
                    ):
                        Wolke.Settings["Deaktivierte-Plugins"].append(
    ...
    def updatePluginCheckboxes(self):
        ...
        if len(pluginNames) > 0:
            self.ui.vlPlugins.addWidget(
                QtWidgets.QLabel("(De-)Aktivieren erfordert einen Neustart!")
            )
