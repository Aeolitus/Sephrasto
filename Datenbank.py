...
import Fertigkeiten
import Objekte
from Hilfsmethoden import (
    Hilfsmethoden,
    VoraussetzungException,
    WaffeneigenschaftException,
)
...
class Datenbank:
    def __init__(self):
        ...
        if Wolke.Settings["Datenbank"]:
            tmp = os.path.join(
                Wolke.Settings["Pfad-Regeln"], Wolke.Settings["Datenbank"]
            )
    ...
    def xmlSchreiben(self):
        ...
        for tal in self.talente:
            ...
            v.set(
                "fertigkeiten", Hilfsmethoden.FertArray2Str(talent.fertigkeiten, None)
            )
        ...
        for we in self.waffeneigenschaften:
            ...
            if not eigenschaft.isUserAdded:
                continue
        ...
        for ma in self.manöver:
            ...
            m.set(
                "voraussetzungen",
                Hilfsmethoden.VorArray2Str(manöver.voraussetzungen, None),
            )
    ...
    def xmlLadenInternal(self, file, refDB):
        ...
        for rem in root.findall("Remove"):
            ...
            if typ == "Vorteil" and name in self.vorteile:
                ...
                typ == "Übernatürliche Fertigkeit"
                and name in self.übernatürlicheFertigkeiten
            ):
            ...
            if removed:
                ...
                logging.warn(
                    "Found remove entry for non-existant element: " + typ + " " + name
                )
        ...
        for vort in vorteilNodes:
            ...
            if vort.get("kommentar"):
                V.kommentarErlauben = (
                    V.variableKosten or int(vort.get("kommentar")) == 1
                )
            else:
            ...
            if V.name in self.vorteile:
                logging.warn(
                    "Vorteil "
                    + V.name
                    + ' exists already in the database, probably a "Remove" entry is missing. '
        ...
        for vort in vorteilNodes:
            ...
            try:
                V.voraussetzungen = Hilfsmethoden.VorStr2Array(
                    vort.get("voraussetzungen"), self
                )
            except VoraussetzungException as e:
                errorStr = (
                    "Error in Voraussetzungen of Vorteil " + V.name + ": " + str(e)
                )
                if notifyError:
        ...
        for tal in talentNodes:
            ...
            try:
                T.voraussetzungen = Hilfsmethoden.VorStr2Array(
                    tal.get("voraussetzungen"), self
                )
            except VoraussetzungException as e:
                errorStr = (
                    "Error in Voraussetzungen of Talent " + T.name + ": " + str(e)
                )
                if notifyError:
                    assert False, errorStr
                logging.warning(errorStr)
        ...
        for fer in fertigkeitNodes:
            ...
            try:
                F.voraussetzungen = Hilfsmethoden.VorStr2Array(
                    fer.get("voraussetzungen"), self
                )
            except VoraussetzungException as e:
                errorStr = (
                    "Error in Voraussetzungen of Fertigkeit " + F.name + ": " + str(e)
                )
                if notifyError:
                    assert False, errorStr
                logging.warning(errorStr)
        ...
        for fer in überFertigkeitNodes:
            ...
            try:
                ...
                errorStr = (
                    "Error in Voraussetzungen of Übernatürliche Fertigkeit "
                    + F.name
                    + ": "
                    + str(e)
                )
        ...
        for ma in manöverNodes:
            ...
            try:
                m.voraussetzungen = Hilfsmethoden.VorStr2Array(
                    ma.get("voraussetzungen"), self
                )
            except VoraussetzungException as e:
                errorStr = (
                    "Error in Voraussetzungen of Manöver " + m.name + ": " + str(e)
                )
                if notifyError:
                    assert False, errorStr
                logging.warning(errorStr)
        ...
        for ffNode in ffNodes:
            ...
            try:
                if ffNode.get("voraussetzungen"):
                    ff.voraussetzungen = Hilfsmethoden.VorStr2Array(
                        ffNode.get("voraussetzungen"), self
                    )
            except VoraussetzungException as e:
                errorStr = (
                    "Error in Voraussetzungen of FreieFertigkeit "
                    + ff.name
                    + ": "
                    + str(e)
                )
        ...
        for wa in self.waffen.values():
            for eig in wa.eigenschaften:
                try:
                    ...
                    errorStr = (
                        "Error in Eigenschaften of Waffe " + wa.name + ": " + str(e)
                    )
                    if notifyError:
                        assert False, errorStr
                    logging.warning(errorStr)
                ...
                for elem in dbType.values():
                    try:
                        Hilfsmethoden.VorStr2Array(
                            Hilfsmethoden.VorArray2Str(elem.voraussetzungen), self
                        )
                    except VoraussetzungException as e:
                        logging.warning(
                            "Error in Voraussetzungen of " + elem.name + ": " + str(e)
                        )
    ...
    def findKampfstile(self):
        kampfstilVorteile = [
            vort
            for vort in self.vorteile.values()
            if vort.typ == 3 and vort.name.endswith(" I")
        ]
