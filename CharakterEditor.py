...
class Editor(object):
    ...
    def finishInit(self):
        ...
        EventBus.doAction(
            "charakter_geladen", {"neu": self.savepath == "", "filepath": self.savepath}
        )
    ...
    def setupMainForm(self, plugins):
        ...
        self.BeschrWrapper = EventBus.applyFilter(
            "class_beschreibung_wrapper", CharakterBeschreibungWrapper.BeschrWrapper
        )()
        self.AttrWrapper = EventBus.applyFilter(
            "class_attribute_wrapper", CharakterAttributeWrapper.AttrWrapper
        )()
        self.FertWrapper = EventBus.applyFilter(
            "class_fertigkeiten_wrapper",
            CharakterFertigkeitenWrapper.FertigkeitenWrapper,
        )()
        self.FreiWrapper = EventBus.applyFilter(
            "class_freiefertigkeiten_wrapper",
            CharakterFreieFertWrapper.CharakterFreieFertWrapper,
        )()
        self.UebernatuerlichWrapper = EventBus.applyFilter(
            "class_uebernatuerlichefertigkeiten_wrapper",
            CharakterUebernatuerlichWrapper.UebernatuerlichWrapper,
        )()
        self.EquipWrapper = EventBus.applyFilter(
            "class_ausruestung_wrapper", CharakterEquipmentWrapper.EquipWrapper
        )()
        self.VortWrapper = EventBus.applyFilter(
            "class_vorteile_wrapper", CharakterVorteileWrapper.CharakterVorteileWrapper
        )()
        self.ItmWrapper = EventBus.applyFilter(
            "class_items_wrapper", CharakterItemsWrapper.CharakterItemsWrapper
        )()
        self.EPWrapper = EventBus.applyFilter(
            "class_ep_wrapper", CharakterEPWrapper.EPWrapper
        )()
        self.NotizWrapper = EventBus.applyFilter(
            "class_notiz_wrapper", CharakterNotizWrapper.CharakterNotizWrapper
        )()

        tabs = []
        tabs.append(
            Tab(10, self.BeschrWrapper, self.BeschrWrapper.formBeschr, "Beschreibung")
        )
        ...
        tabs.append(
            Tab(50, self.FertWrapper, self.FertWrapper.formFert, "Fertigkeiten")
        )
        tabs.append(
            Tab(60, self.FreiWrapper, self.FreiWrapper.formFert, "Freie Fertigkeiten")
        )
        tabs.append(
            Tab(
                70,
                self.UebernatuerlichWrapper,
                self.UebernatuerlichWrapper.formFert,
                "Übernatürliches",
            )
        )
        ...
        if not self.ignoreModified:
            ...
            if Wolke.Char.EPtotal < Wolke.Char.EPspent:
                self.ui.spinRemaining.setStyleSheet(
                    "QSpinBox { background-color: rgb(200,50,50) }"
                )
            else:
                self.ui.spinRemaining.setStyleSheet("")
    ...
    def saveButton(self):
        ...
        spath, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Charakter speichern...", startDir, "XML-Datei (*.xml)"
        )
        if spath == "":
            return
        if ".xml" not in spath:
        ...
        if spath.endswith("datenbank.xml"):
            ...
            infoBox.setInformativeText(
                "Ich empfehle, die Regelbasis nicht mit einem Charakter zu überschreiben. \
    ...
    def quicksaveButton(self):
        ...
        if self.savepath == "":
            ...
            try:
                ...
                infoBox.setText("Speichern des Charakters fehlgeschlagen!")
                infoBox.setInformativeText(
                    "Beim Speichern des Charakters ist ein Fehler aufgetreten!\n\
Fehlercode: "
                    + str(Wolke.Fehlercode)
                    + "\n\
Fehlermeldung: "
    ...
    def pdfButton(self):
        ...
        if Wolke.Settings["Bogen"] == "Frag immer nach":
            ...
            messagebox.addButton(
                QtWidgets.QPushButton("  Standard Ilaris-Charakterbogen  "),
                QtWidgets.QMessageBox.AcceptRole,
            )
            messagebox.addButton(
                QtWidgets.QPushButton("  Lange Version von Gatsu  "),
                QtWidgets.QMessageBox.AcceptRole,
            )
            messagebox.addButton(
                QtWidgets.QPushButton("Abbrechen"), QtWidgets.QMessageBox.RejectRole
        ...
        if result == 0 or Wolke.Settings["Bogen"] == "Standard Ilaris-Charakterbogen":
            charakterBogen = CharakterbogenInfo(
            ...
            charakterBogen = CharakterbogenInfo(
                filePath="Charakterbogen_lang.pdf",
                maxVorteile=24,
                maxFreie=28,
                maxFertigkeiten=28,
                seitenProfan=3,
        ...
        self.pdfMeister.setCharakterbogen(
            EventBus.applyFilter("set_charakterbogen", charakterBogen)
        )
        ...
        if not os.path.isfile(self.pdfMeister.CharakterBogen.filePath):
            ...
            messagebox.setText(
                "Konnte "
                + self.pdfMeister.CharakterBogen.filePath
                + " nicht im Installationsordner finden"
            )
        ...
        startDir = os.path.join(
            startDir,
            Wolke.Char.name or os.path.splitext(os.path.basename(self.savepath))[0],
        )

        # Let the user choose a saving location and name
        spath, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Charakterbogen erstellen...", startDir, "PDF-Datei (*.pdf)"
        )
        if spath == "":
            return
        if ".pdf" not in spath:
