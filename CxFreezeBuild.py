...
env_plugins_path = os.path.join(
    env_python_path, "Lib", "site-packages", "PyQt5", "Qt5", "plugins"
)
...
env_bin_path = os.path.join(
    env_python_path, "Lib", "site-packages", "PyQt5", "Qt5", "bin"
)
...
setup(
    ...
    version=str(Version._sephrasto_version_major)
    + "."
...
def includePlugin(name):
    ...
    for i in files:
        ...
        if (
            not os.path.basename(file) == "__pycache__"
            and not os.path.basename(file) == "UI"
        ):
