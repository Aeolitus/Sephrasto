...
class CharakterFreieFertigkeitenPickerWrapper(object):
    def __init__(self, fertigkeit=None):
        ...
        self.ui.treeFerts.itemDoubleClicked.connect(
    ...
    def formatName(self, freieFertigkeit):
        return EventBus.applyFilter(
            "format_freiefertigkeit_name",
            freieFertigkeit.kategorie + ": " + freieFertigkeit.name,
            {"fertigkeit": freieFertigkeit},
        )
    ...
    def populateTree(self):
        ...
        for kategorie in kategorien:
            ...
            for fert in Wolke.DB.freieFertigkeiten:
                ...
                if (
                    ...
                    and (not self.ui.nameFilterEdit.text().lower() in fert.lower())
                    and (not self.ui.nameFilterEdit.text().lower() in kategorie.lower())
                ):
                    continue
                if fert != self.current and self.formatName(
                    Wolke.DB.freieFertigkeiten[fert]
                ) in [fert.name for fert in Wolke.Char.freieFertigkeiten]:
                    continue
                if Wolke.Char.voraussetzungenPrüfen(
                    Wolke.DB.freieFertigkeiten[fert].voraussetzungen
                ):
                    ferts.append(fert)
            ferts.sort()
            if len(ferts) == 0:
            ...
            for el in ferts:
                ...
                )  # store key of talent in user data
        ...
        if self.current in Wolke.DB.freieFertigkeiten:
            ...
            found = self.ui.treeFerts.findItems(
                name, QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive
            )
            if len(found) > 0:
                self.ui.treeFerts.setCurrentItem(
                    found[0], 0, QtCore.QItemSelectionModel.Select
                )
        elif (
            self.ui.treeFerts.topLevelItemCount() > 0
            and self.ui.treeFerts.topLevelItem(0).childCount() > 0
        ):
            self.ui.treeFerts.setCurrentItem(
                self.ui.treeFerts.topLevelItem(0).child(0),
                0,
