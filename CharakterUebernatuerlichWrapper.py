...
class UebernatuerlichWrapper(QtCore.QObject):
    ...
    def load(self):
        ...
        temp = [
            ...
            if Wolke.Char.voraussetzungenPrüfen(
                Wolke.DB.übernatürlicheFertigkeiten[el].voraussetzungen
            )
        ]
        if temp != self.availableFerts:
            ...
            self.availableFerts.sort(
                key=lambda x: (Wolke.DB.übernatürlicheFertigkeiten[x].printclass, x)
            )

            rowIndicesWithLinePaint = []
            count = 0
            if len(self.availableFerts) > 0:
                lastPrintclass = Wolke.DB.übernatürlicheFertigkeiten[
                    self.availableFerts[0]
                ].printclass
                for el in self.availableFerts:
                    if (
                        Wolke.DB.übernatürlicheFertigkeiten[el].printclass
                        != lastPrintclass
                    ):
            ...
            self.uiFert.tableWidget.setItemDelegate(
                FertigkeitItemDelegate(rowIndicesWithLinePaint)
            )
            ...
            for el in self.availableFerts:
                ...
                if el not in Wolke.Char.übernatürlicheFertigkeiten:
                    Wolke.Char.übernatürlicheFertigkeiten.update(
                        {el: Wolke.DB.übernatürlicheFertigkeiten[el].__deepcopy__()}
                    )
                ...
                self.pdfRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ] = QtWidgets.QCheckBox()
                self.pdfRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].setStyleSheet("margin-left:10; margin-right:10;")
                self.pdfRef[Wolke.Char.übernatürlicheFertigkeiten[el].name].setChecked(
                    Wolke.Char.übernatürlicheFertigkeiten[el].addToPDF
                )
                self.pdfRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].stateChanged.connect(
                    lambda state, name=Wolke.Char.übernatürlicheFertigkeiten[
                        el
                    ].name: self.addToPDFClicked(name, state)
                )
                self.uiFert.tableWidget.setCellWidget(
                    count,
                    0,
                    self.pdfRef[Wolke.Char.übernatürlicheFertigkeiten[el].name],
                )

                self.uiFert.tableWidget.setItem(
                    count,
                    1,
                ...
                self.spinRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].installEventFilter(self.mwp)
                self.spinRef[Wolke.Char.übernatürlicheFertigkeiten[el].name].setMinimum(
                    0
                )
                self.spinRef[Wolke.Char.übernatürlicheFertigkeiten[el].name].setMaximum(
                    Wolke.Char.übernatürlicheFertigkeiten[el].maxWert
                )
                self.spinRef[Wolke.Char.übernatürlicheFertigkeiten[el].name].setValue(
                    Wolke.Char.übernatürlicheFertigkeiten[el].wert
                )
                self.spinRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ...
                self.spinRef[
                    ...
                    lambda state, name=Wolke.Char.übernatürlicheFertigkeiten[
                        el
                    ].name: self.spinnerClicked(name)
                )
                self.uiFert.tableWidget.setCellWidget(
                    count,
                    2,
                    self.spinRef[Wolke.Char.übernatürlicheFertigkeiten[el].name],
                )
                ...
                self.labelRef[Wolke.Char.übernatürlicheFertigkeiten[el].name].setText(
                    str(len(Wolke.Char.übernatürlicheFertigkeiten[el].gekaufteTalente))
                )
                self.labelRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].setAlignment(QtCore.Qt.AlignCenter)
                ...
                self.layoutRef[
                    ...
                    self.labelRef[Wolke.Char.übernatürlicheFertigkeiten[el].name]
                )
                self.buttonRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ...
                self.buttonRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].setMaximumSize(QtCore.QSize(25, 20))
                self.buttonRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].clicked.connect(
                    lambda state, name=Wolke.Char.übernatürlicheFertigkeiten[
                        el
                    ].name: self.addClicked(name)
                )
                self.layoutRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ].addWidget(
                    self.buttonRef[Wolke.Char.übernatürlicheFertigkeiten[el].name]
                )
                self.widgetRef[
                    Wolke.Char.übernatürlicheFertigkeiten[el].name
                ] = QtWidgets.QWidget()
                self.widgetRef[
                    ...
                    self.layoutRef[Wolke.Char.übernatürlicheFertigkeiten[el].name]
                )
                self.uiFert.tableWidget.setCellWidget(
                    count,
                    3,
                    self.widgetRef[Wolke.Char.übernatürlicheFertigkeiten[el].name],
                )
                ...
                self.rowRef.update(
                    {Wolke.Char.übernatürlicheFertigkeiten[el].name: count}
                )
                count += 1
    ...
    def tableClicked(self):
        if not self.currentlyLoading:
            tmp = self.uiFert.tableWidget.item(
                self.uiFert.tableWidget.currentRow(), 1
            ).text()
    ...
    def updateInfo(self):
        if self.currentFertName != "":
            ...
            self.uiFert.labelAttribute.setText(
                fert.attribute[0] + "/" + fert.attribute[1] + "/" + fert.attribute[2]
            )
    ...
    def updateTalents(self):
        if self.currentFertName != "":
            ...
            for el in fert.gekaufteTalente:
                talStr = (
                    Wolke.DB.talente[el]
                    .getFullName(Wolke.Char)
                    .replace(self.currentFertName + ": ", "")
                )
                costStr = ""
                if not el in Wolke.Char.talenteVariable:
                    costStr = (
                        " ("
                        + str(Wolke.Char.getTalentCost(el, fert.steigerungsfaktor))
                        + " EP)"
                    )
    ...
    def updateTalentRow(self):
        for i in range(self.uiFert.tableWidget.rowCount()):
            ...
            self.labelRef[fert].setText(
                str(len(Wolke.Char.übernatürlicheFertigkeiten[fert].gekaufteTalente))
            )
