...
class Fertigkeit(Steigerbar):
    def __init__(self):
        ...
        self.talentMods = (
            {}
        )  # for vorteil scripts, { talentnname1 : { condition1 : mod1, condition2 : mod2, ... }, talentname2 : {}, ... }
        self.kampffertigkeit = 0
        # 0 = nein, 1 = nahkampffertigkeit, 2 = sonstige kampffertigkeit
    ...
    def aktualisieren(self):
        self.attributswerte = [
            Wolke.Char.attribute[self.attribute[0]].wert,
        ...
        self.basiswert = (
            round(sum(self.attributswerte) / 3 + 0.0001) + self.basiswertMod
        )
...
class Vorteil:
    ...
    def getFullName(self, char, forceKommentar=False):
        ...
        if self.name in char.vorteileVariable:
            addKommentar = char.vorteileVariable[self.name].kommentar != "" and (
                forceKommentar or not ("$kommentar$" in self.cheatsheetBeschreibung)
            )
            addEP = self.variableKosten
